...
public class AlertDialogLayout extends C0158i0 {
    public AlertDialogLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!mo4035C(widthMeasureSpec, heightMeasureSpec)) {
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        }
    }

    /* renamed from: C */
    public final boolean mo4035C(int widthMeasureSpec, int heightMeasureSpec) {
        int childHeightSpec;
        ...
        for (int i3 = 0; i3 < count; i3++) {
            ...
            if (child.getVisibility() != 8) {
                int id = child.getId();
                if (id == C0008f.topPanel) {
                    topPanel = child;
                } else if (id == C0008f.buttonPanel) {
                    buttonPanel = child;
                } else if ((id != C0008f.contentPanel && id != C0008f.customPanel) || middlePanel != null) {
                    return false;
                } else {
                    middlePanel = child;
                }
        ...
        if (buttonPanel != null) {
            ...
            buttonHeight = m3844B(buttonPanel);
    ...
    /* renamed from: l */
    private void m3845l(int count, int heightMeasureSpec) {
        int uniformMeasureSpec = View.MeasureSpec.makeMeasureSpec(getMeasuredWidth(), 1073741824);
        for (int i = 0; i < count; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                C0158i0.C0159a lp = (C0158i0.C0159a) child.getLayoutParams();
                if (lp.width == -1) {
    ...
    /* renamed from: B */
    public static int m3844B(View v) {
        int minHeight = C0460r.m2293w(v);
        if (minHeight > 0) {
            return minHeight;
        }
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        while (i3 < count) {
            ...
            if (child == null || child.getVisibility() == 8) {
                ...
                int b = C0444c.m2174b(layoutGravity, layoutDirection) & 7;
                ...
                if (alertDialogLayout.mo1158t(i3) != 0) {
                    childTop += dividerHeight;
                }
                int childTop2 = childTop + lp.topMargin;
                int i5 = layoutGravity;
                i2 = i3;
                m3843A(child, childLeft, childTop2, childWidth, childHeight);
    ...
    /* renamed from: A */
    private void m3843A(View child, int left, int top, int width, int height) {
